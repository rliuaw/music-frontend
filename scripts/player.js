 ...
 * {
 *     "title": "永遠の巫女",
 *     "file": "/audio/th01/2.永遠の巫女.mp3",
 *     "howl": null,
 *     "info": {
 *         "character": "博麗靈夢",
 *         "chorus_end_time": 95,
 *         "chorus_start_time": 47,
 *         "keyword": "東方靈異伝 LUNATIC stage1",
 *         "name": "2.永遠の巫女.mp3",
 *         "path": "/audio/th01/2.永遠の巫女.mp3"
 *     },
 *     "chorusStartTime": 47,
 *     "chorusEndTime": 95
 * }
...
Player.prototype = {
  ...
  load: function (index) {
    ...
    if (data.howl) {
      ...
      sound = data.howl = new Howl({
        ...
        // src: ['.' + data.file],
        html5: true, // Force to HTML5 so that the audio can stream in (best for large files).
        // preload: true, // Force to HTML5 so that the audio can stream in (best for large files).
        onplay: function () {
          // Display the duration.
          duration.innerHTML = self.formatTime(Math.round(sound.duration()));
  ...
  skip: function (direction) {
    ...
    if (direction === 'prev' && self.playlist[self.index].howl && self.playlist[self.index].howl.seek() <= 3) {
      ...
      index = self.nextIndex || calcNextIndex(self.index);
      self.skipTo(index);
      self.nextIndex = calcNextIndex(self.index)
      self.load(self.nextIndex);
    }
  },
